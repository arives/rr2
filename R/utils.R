...
binaryPGLMM <- function(formula, data = list(), phy, s2.init = 0.1, B.init = NULL, tol.pql = 10^-6, maxit.pql = 200, maxit.reml = 100) {
    ...
    results <- list(formula = formula, B = B, B.se = B.se, B.cov = B.cov, B.zscore = B.zscore, B.pvalue = B.pvalue, s2 = s2, P.H0.s2 = P.H0.s2, mu = mu, b = b, 
        B.init = B.init, X = X, y = y, phy = phy, data = data, H = H, VCV = Vphy, V = V, convergeflag = convergeflag, iteration = iteration, converge.test.s2 = converge.test.s2, 
...
phyloglm <- function(formula, data = list(), phy, method = c("logistic_MPLE", "logistic_IG10", "poisson_GEE"), btol = 10, log.alpha.bound = 4, start.beta = NULL, 
    ...
    transf.branch.lengths <- function(B, lL) {
        ...
        tmp = .C("transbranchlengths_IvesGarland2010", as.integer(N), as.integer(des), as.integer(anc - n), as.integer(lok), as.double(distFromRoot), as.integer(externalEdge), 
            as.double(mu), as.double(p), as.double(alpha), as.double(D), el = as.double(1:N), di = as.double(1:n))
        edge.length = tmp$el
        diag = tmp$di
    ...
    three.point.compute <- function(trans, y, X) {
        ole = 4 + 2 * dk + dk * dk
        tmp = .C("threepoint", as.integer(N), as.integer(n), as.integer(phy$Nnode), as.integer(1), as.integer(dk), as.integer(ROOT), as.double(trans[[2]]), as.double(trans[[1]]), 
            as.integer(des), as.integer(anc), as.double(as.vector(y)), as.double(as.vector(X)), result = double(ole))$result
        return(list(vec11 = tmp[2], y1 = tmp[3], yy = tmp[4], X1 = tmp[5:(4 + dk)], XX = matrix(tmp[(5 + dk):(ole - dk)], dk, dk), Xy = tmp[(ole - dk + 1):ole], 
            logd = tmp[1]))
    }
    ...
    npllh <- function(par) {
        ...
        infoM = comp$XX
        llk <- .C("logistreglikelihood", as.integer(N), as.integer(n), as.integer(phy$Nnode), as.integer(ROOT), as.double(original.edge.length), as.integer(des), 
            as.integer(anc), as.integer(as.vector(y)), as.double(as.vector(mu)), as.integer(dk), as.double(exp(-par[dk + 1])), loglik = double(1))$loglik
        if (dk == 1) 
            pllik = llk + log(abs(infoM))/2 else pllik = llk + log(det(infoM))/2
        -pllik
    }
    llh <- function(mu, alpha) {
        .C("logistreglikelihood", as.integer(N), as.integer(n), as.integer(phy$Nnode), as.integer(ROOT), as.double(original.edge.length), as.integer(des), as.integer(anc), 
            as.integer(as.vector(y)), as.double(as.vector(mu)), as.integer(dk), as.double(alpha), loglik = double(1))$loglik
    }
    ...
    if (method %in% c("logistic_MPLE", "logistic_IG10")) {
        if (is.null(start.alpha)) 
            ...
            if (abs(startlL - log(Tmax)) >= log.alpha.bound) {
                ...
                tmp = paste(tmp, signif(exp(-log.alpha.bound)/Tmax, 3), ",", signif(exp(log.alpha.bound)/Tmax, 3), " (Tmax=", Tmax, ").", "\n  Change start.alpha or increase log.alpha.bound.", 
                  sep = "")
                stop(tmp)
            }
        }
    ...
    if ((boot > 0) && (method %in% c("logistic_MPLE", "logistic_IG10"))) {
        options(warn = -1)
        bootobject <- phylolm::rbinTrait(n = boot, phy = phy, beta = results$coefficients, alpha = results$alpha, X = X, model = "LogReg")
        ...
        for (i in 1:boot) {
            ...
            if (method == "logistic_MPLE") {
                bootfit <- try(optim(par = c(startB, startlL), fn = npllh, method = opt.method, control = list(factr = 1e+12)), silent = TRUE)
                if (!inherits(bootfit, "try-error")) {
