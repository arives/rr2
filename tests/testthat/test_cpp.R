context("testing R2.pred functions with loops written in c++")
...
test_that("PGLS: c++ version give the same results with the R version", {
    ...
    mod <- phylolm::phylolm(y_pgls ~ x_trait, phy = phy, data = d, model = "lambda", starting.value = 0.98 * lam.x + 0.01)
    mod.r <- lm(y_pgls ~ x_trait, data = d)
    
    expect_equal(R2.pred(mod, mod.r, phy = phy, cpp = T), R2.pred(mod, mod.r, phy = phy, cpp = F))
})
